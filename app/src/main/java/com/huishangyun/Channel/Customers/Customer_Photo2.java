...
public class Customer_Photo2 extends BaseActivity implements OnUpLoadResult{
...
//	private ProgressDialog pDialog;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		init();
		clickListener();
		viewpager.setCurrentItem(CustomerInfo.currentItem);
		
		photo_nub.setText(CustomerInfo.currentItem + 1 + "/" + CustomerInfo.path.size());
	}
...
//        Log.e(TAG, "加载的图片大小：" + bitmap.getByteCount());
	...
	private void init() {
		...
		for (int i = 0; i < CustomerInfo.path.size(); i++) {
			String path = CustomerInfo.path.get(i);
	...
	private void clickListener() {
		again.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				CallSystemApp.callCamera(Customer_Photo2.this, path01);
		...
		delete.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				switch (CustomerInfo.currentItem) {
				case 0:					
					CustomerInfo.path.remove(0);
					CustomerInfo.path_serve.remove(0);
					Log.i("==============", "删除后的list大小："+CustomerInfo.path.size());
					if (CustomerInfo.path.size() > 0) {
						init();
					} else {
						finish();
					}
				...
				case 1:
					CustomerInfo.path.remove(1);
					CustomerInfo.path_serve.remove(1);
					Log.i("==============", "删除后的list大小："+CustomerInfo.path.size());
					if (CustomerInfo.path.size() > 0) {
						init();
				...
				case 2:
					CustomerInfo.path.remove(2);
					CustomerInfo.path_serve.remove(2);
					Log.i("==============", "删除后的list大小："+CustomerInfo.path.size());
					if (CustomerInfo.path.size() > 0) {
						init();
...
//			setImageToNet(path01, path01_name);
	...
	private class MyPageChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			...
			Log.i("-------------------","当前的currentItem:"+CustomerInfo.currentItem);
			
			
			CustomerInfo.currentItem = position;
			oldPosition = position;
			photo_nub.setText(position + 1 + "/" + CustomerInfo.path.size());
   ...
   /**
	...
	private void setImageToNet(String path, String FileName) {
	   ...
	   upLoadImgSignText.setUpLoadResult(Customer_Photo2.this);
	...
	public void onUpLoadResult(String FileName, String FilePath,
		...
		if (isSuccess) {
			...
			case 0:
				CustomerInfo.path.set(0, path01);
				CustomerInfo.path_serve.set(0, FilePath);
				path01 = "";
				path01_name = "";
				init();
				viewpager.setCurrentItem(0);
			...
			case 1:
				CustomerInfo.path.set(1, path01);
				CustomerInfo.path_serve.set(1, FilePath);
				path01 = "";
				path01_name = "";
				init();
				viewpager.setCurrentItem(1);
			...
			case 2:
				CustomerInfo.path.set(2, path01);
				CustomerInfo.path_serve.set(2, FilePath);
				path01 = "";
				path01_name = "";
				init();
...
//			CustomerInfo.path_serve.add(FilePath);			
