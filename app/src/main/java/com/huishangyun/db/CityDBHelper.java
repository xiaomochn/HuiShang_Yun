...
public class CityDBHelper extends SQLiteOpenHelper {
    ...
    //如果你想把数据库文件存放在SD卡的话
    private static String DB_PATH = Constant.SAVE_DATABASE_PATH;
     ...
     */
    public CityDBHelper(Context context, String name, CursorFactory factory, int version) {
    ...
    public void createDataBase() throws IOException {
        ...
        if (dbExist) {
            ...
            L.e("数据库已存在");
            //判断是否要升级城市数据库
            if (!MyApplication.getInstance().getSharedPreferences().getBoolean(Constant.HUISHANG_CITYDB_COPY, false)) {
                L.e("从新拷贝数据库");
                doCopy();
                MyApplication.getInstance().getSharedPreferences().edit().putBoolean(Constant.HUISHANG_CITYDB_COPY, true).commit();
            }
        } else {
            //创建数据库
            L.e("创建数据库");
